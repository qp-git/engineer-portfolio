...
- クレジットカード明細をもとにした家計簿データをDBに保存し、SQLで支出傾向を分析する仕組みを作成予定です。カテゴリ別・月別・固定費/変動費別の集計に加え、GCP BigQueryを使ったクラウド上でのSQL分析、権限管理、コスト管理も検証したいと考えています。

* 古いノートPCにLubuntuを導入し、ローカル環境とクラウドを組み合わせたハイブリッド構成の検証にも取り組みたいと考えています。現時点では具体的な構成は未定ですが、オンプレミス寄りのLinuxサーバーとクラウド環境を接続し、ネットワーク設計、運用管理、監視、セキュリティ面の違いを実際に確認できるテーマとして発展させたいです。
