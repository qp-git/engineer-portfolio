...
ユーザーが Alexa に話しかけると、Alexa Skill 側で発話内容が解析され、Lambda にリクエストが送られます。
...
通常の Web API では、外部から Lambda を呼び出す入口として API Gateway を使うことがあります。しかし Alexa Skill では、Alexa Skills Kit のエンドポイントとして Lambda 関数の ARN を指定できるため、API Gateway を前段に置かなくても構成できます。
...
この構成により、Alexa Skill が Lambda の入口となり、Lambda が処理結果を Alexa に返す流れをシンプルに整理できました。

## Lambda から外部 API を呼ぶ流れ

Lambda 側では、外部 API を呼び出して必要な情報を取得します。
