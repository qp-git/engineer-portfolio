...
ユーザーが Alexa に話しかけると、Alexa Skill 側で発話内容が解析されます。
...
外部 API 呼び出しでは、Lambda のタイムアウトや fetch 側のタイムアウト、API 側の応答速度を考慮する必要があります。
...
音声 UI では画面に情報を一覧表示できないため、短く、自然で、聞き取りやすい文章にすることが重要です。
