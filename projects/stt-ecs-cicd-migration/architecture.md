...
## 1. 移行前: GitHub Actions側が本番入口を受けている状態
...
```mermaid
flowchart LR
    Check[Pipeline事前確認] -->|HTTP:81| ALB[ALB]
...
```mermaid
flowchart LR
    GitHub[GitHub] --> Pipeline[CodePipeline]
    Pipeline --> Build[Build<br>CodeBuild]
    Build --> ECR[ECR]
    ECR --> Deploy[Deploy<br>ECS Deploy Action]
    Deploy --> ECS[ECS Service]

    Deploy --> Smoke[Smoke Test<br>CodeBuild]
    Smoke -->|HTTPS 本番URL| ALB[ALB]
    ALB --> TG[Pipeline側 Target Group]
    TG --> Task[ECS Task<br>Flask App]
    Task --> Secrets[Secrets Manager]
