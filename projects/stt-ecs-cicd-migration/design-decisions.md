...
このSmoke Testが成功するためには、ALB HTTPS:443がPipeline側Target Groupへ転送され、ECS Task上のアプリがSecrets Managerから注入されたAPIキーを使ってOpenAI APIへリクエストできる必要があります。
...
<!-- ALT_ALB_DECISION_START -->
## 別ALB構成にしなかった理由

より分離度を高める場合、新旧でALBを分け、別DNS名で新環境を検証した後にRoute 53のAliasを切り替える方法も考えられます。

```mermaid
flowchart LR
    User[User] --> DNS[Route 53 Alias]
    DNS --> OldALB[既存ALB]
    OldALB --> OldSvc[Actions側 ECS Service]

    NewDNS[検証用DNS] --> NewALB[新ALB]
    NewALB --> NewSvc[Pipeline側 ECS Service]

    DNS -. 切替 .-> NewALB
```

この構成は、新旧環境をALB単位で分離できるため、検証環境としては分かりやすいです。

一方で、今回の学習・検証環境では、追加ALB、DNS切替、証明書、Security Group、コストの管理が増えます。

そのため、今回は別ALB構成ではなく、同じALB内でTarget Groupと一時的な検証用Listenerを分ける方式を採用しました。

この方式により、構成複雑性とコストを抑えつつ、本番HTTPS:443を移行直前まで既存Actions側に維持し、HTTP:81でPipeline側を事前確認できる状態を作りました。

<!-- ALT_ALB_DECISION_END -->
